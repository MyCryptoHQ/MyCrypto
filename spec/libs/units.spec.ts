import { fromWei, Wei, toWei, toTokenBase, fromTokenBase } from 'libs/units';
const Units = {
  wei: '1',
  kwei: '1000',
  ada: '1000',
...
describe('Units', () => {
  describe('ethereum units', () => {
    describe('ether', () => {
      ...
      it('should equal one ether', () => {
        expect(ether.value).toEqual('1');
      });

      it('should equal 1 * 10^18 wei', () => {
        const converted = toWei(ether);
    ...
    describe('gwei', () => {
      ...
      it('should equal 21 gwei', () => {
        expect(gwei.value).toEqual('21');
      });
      it('should equal 21 * 10^9 wei', () => {
        const converted = toWei(gwei);
    ...
    describe('kwei', () => {
      ...
      it('should equal 1.623 kwei', () => {
        expect(kwei.value).toEqual('1.623');
      });
      it('should equal 1.623  * 10^3 wei', () => {
        const converted = toWei(kwei);
  ...
  describe('token units', () => {
    describe('token 1', () => {
      ...
      const decimal = 18;
      const tokenBase = toTokenBase({ value: tokens, decimal });
      ...
      it('should equal 732156.34592016', () => {
        expect(
          fromTokenBase({ value: tokenBase.toString(), decimal }).value
        ).toEqual(tokens);
      });
    });
    describe('token 2', () => {
      const tokens = '8000';
      const decimal = 8;
      const converted = fromTokenBase({ value: tokens, decimal });
      it('should equal 0.00008', () => {
        expect(converted.value).toEqual('0.00008');
      });
      it('should equal 8000', () => {
        expect(toTokenBase(converted));
      });
    });
  });
});
