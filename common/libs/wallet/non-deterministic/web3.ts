...
export default class Web3Wallet implements IFullWallet {
  ...
  public async sendTransaction(transaction: ExtendedRawTransaction): Promise<string> {
    const state = configuredStore.getState();
    const nodeLib: Web3Node | INode = getNodeLib(state);
    ...
    const web3Tx = {
      ...
      to,
      value,
      gas: gasLimit instanceof BN ? sanitizeHex(gasLimit.toString(16)) : gasLimit,
      gasPrice: gasPrice instanceof BN ? sanitizeHex(gasPrice.toString(16)) : gasPrice,
      data,
      nonce
    };
