...
function* shaBid(): SagaIterator {
  const requestChan = yield actionChannel(
    types.ENSActions.ENS_SHA_BID_REQUESTED,
    buffers.sliding(1)
  );

  while (true) {
    const { payload }: types.ShaBidRequested = yield take(requestChan);

    try {
      const node: INode = yield select(configNodesSelectors.getNodeLib);

      const result = yield race({
        shaBidData: call(helpers.shaBidRequest, payload, node),
        err: call(delay, 10000)
      });
      ...
      yield put(actions.shaBidSucceeded(shaBidData, false));
    } catch (e) {
      const shaBidFailAction = actions.shaBidFailed('error', false);
      yield put(shaBidFailAction);
      yield put(
        notificationsActions.showNotification('danger', e.message || 'Could not get SHA Bid', 5000)
      );
    }
  }
}
