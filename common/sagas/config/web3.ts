...
export function* unlockWeb3(): SagaIterator {
  try {
    ...
    const networkId: string = yield apply(nodeLib, nodeLib.getNetVersion);
    ...
    const wallet = new Web3Wallet(address, stripWeb3Network(network), Number.parseInt(networkId));
