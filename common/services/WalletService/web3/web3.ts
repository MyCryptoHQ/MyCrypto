...
import { isWeb3Node, setupWeb3Node, Web3Service } from 'services/EthService';
...
import { INetwork, NodeOptions, NodeType } from 'typeFiles';
...
export const initWeb3Node = async () => {
  ...
  const network: INetwork | undefined = getNetworkByChainId(parseInt(chainId, 10));
  if (!network) {
