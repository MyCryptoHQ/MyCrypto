import { isDevelopment, generateUUID } from 'utils';
import {
  Fiats,
  ContractsData,
  AssetsData,
...
export const initNetworks = () => {
  ...
  allNetworks.map((networkId: NetworkId) => {
    const newContracts: [string, IContract][] = Object.entries(newStorage.contracts).filter(
    ...
    const newLocalNetwork: INetwork = {
