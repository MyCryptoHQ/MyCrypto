...
import { ExtendedAccount as IExtendedAccount, Network } from 'v2/types';
...
function AccountDropdown({ accounts, name, value, values, onSelect }: IAccountDropdown) {
  let relevantAccounts: IExtendedAccount[] = [];
  if (values.asset && values.asset.network) {
    relevantAccounts = accounts.filter((account: IExtendedAccount): boolean => {
