...
const Satochip: WalletLib = {
  ...
  signTransaction(tx, dpath) {
    ...
    console.log('Satochip: tx:' + unsignedTx.toJSON());
    console.log('Satochip: chainId:' + tx.chainId);
    console.log('Satochip: serializedTx: ' + unsignedTx.serialize().toString('hex'));
    console.log('Satochip: serializedTx hash(false) : ' + unsignedTx.hash(false).toString('hex'));
    console.log('Satochip: serializedTx hash(true) : ' + unsignedTx.hash(true).toString('hex'));

    const msg: any = {
      requestID: SatochipSession.requestID++,
      action: 'sign_tx_hash',
      tx: unsignedTx.serialize().toString('hex'),
      hash: unsignedTx.hash(true).toString('hex'),
      path: dpath,
      alt: 'etherlike',
      chainid: tx.chainId
    };
      ...
      const response = new Promise((resolve2) => {
        ...
        try {
          if (parseInt(res.v, 16) <= 1) {
            ...
            res.v = parseInt(res.v, 16) + 2 * tx.chainId + 35; //res.v += 2 * chainId + 35;
          }
          /*           signedTx.v = res.v;
          ...
          const signedTx = new EthTx(
            {
              ...tx,
              v: res.v,
              r: Buffer.from(res.r, 'hex'),
              s: Buffer.from(res.s, 'hex')
            },
            { chain: tx.chainId, hardfork: 'tangerineWhistle' }
          );
          console.log('Satochip: serializedTx TEST2');
          console.log('Satochip: tx:' + signedTx.toJSON());
          console.log('Satochip: txraw:' + signedTx.raw.toString());
