...
export function* vaultSaga() {
  yield all([
    ...
    takeLatest(decrypt.type, encryptionWorker)
  ]);
}
...
export function* encryptionWorker({ payload: password }: PayloadAction<string>) {
  ...
  yield put({ type: ActionT.RESET, payload: initialLegacyState });
}

export function* decryptionWorker({ payload: password }: PayloadAction<string>) {
  const passwordHash = yield call(hashPassword, password);
