  ...
  const wallets = (() => {
    switch (walletType) {
      ...
      case WalletId.SATOCHIP:
        console.log(
          'Satochip: src/features/AddAccount/AddAccountForm.reducer.tsx: in handleUnlock(): payload=',
          payload
        ); //debugSatochip
        console.log(
          'Satochip: src/features/AddAccount/AddAccountForm.reducer.tsx: in handleUnlock(): derivationPath=',
          payload.dPath + '/' + payload.index.toString()
        ); //debugSatochip
        return [
          {
            address: payload.address,
            dPath: payload.dPath + '/' + payload.index.toString()
          }
        ];
