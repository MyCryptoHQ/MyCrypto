...
describe('constructTxTypeConfig', () => {
  ...
  testCases.forEach((testCase) => {
    test(`correctly handles action to determine derived tx label for txType ${testCase.inputType}_${testCase.inputProtocol}`, () => {
      const result = constructTxTypeConfig({
        type: testCase.inputType,
        protocol: testCase.inputProtocol
      });
...
describe('sumValueTransfers', () => {
  ...
  it('correctly sums valueTransfers with balance', () => {
    const expected = '50';
    const actual = sumValueTransfers(
      fValueTransfers.map((t) => ({ ...t, amount: '5' })),
      getAssetRate
    );
  ...
  it('correctly sums valueTransfers with balance and ignores valueTransfers without balance', () => {
    const expected = '50';
    const actual = sumValueTransfers(
      [...fValueTransfers.map((t) => ({ ...t, amount: '5' })), ...fValueTransfers],
      getAssetRate
    );
    expect(actual.toString()).toBe(expected);
  });
